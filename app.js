...
function setupsearch(){
    ...
    // contact filter
    const filtered = phonBook.filter(contact =>
...
function localContactToForm(index){
    if (index===null || index <0 || index>=phonBook.length)
        return;
...
btnSave.addEventListener('click',function(){
    ...
    if (name === '' || phon === '') {
        if (name === '') checkError.innerHTML = 'نام و نام خانوادگی نباید خالی باشد.';
    ...
    const newcontact={
        name: name , phone: phon , company: company|| 'شخصی'
    };
...
btndelete.addEventListener('click',function(){
    ...
    const confrimDelete=confirm(`آیا از حذف ${phonBook[editIndex].name} مطمئن هستید ؟`)
